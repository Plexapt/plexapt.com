;(function () {
	...
	var burgerMenu = function() {
		...
		$('body').on('click', '.js-plexapt-nav-toggle', function(event){
	...
	var clickMenu = function() {
		...
		$('a:not([class="external"])').click(function(event){
		    ...
		    if ( navbar.is(':visible')) {
		    	...
		    	$('.js-plexapt-nav-toggle').removeClass('active');
		    }
	...
	var windowScroll = function() {
		...
		$(window).scroll(function(event){
		   	...
		   	var header = $('#plexapt-header'),
				scrlTop = $(this).scrollTop();

			if ( scrlTop > 500 && scrlTop <= 2000 ) {
				header.addClass('navbar-fixed-top plexapt-animated slideInDown');
			} else if ( scrlTop <= 500) {
				if ( header.hasClass('navbar-fixed-top') ) {
					header.addClass('navbar-fixed-top plexapt-animated slideOutUp');
					setTimeout(function(){
						header.removeClass('navbar-fixed-top plexapt-animated slideInDown slideOutUp');
					}, 100 );
				}
			}
	...
	var servicesAnimate = function() {
		...
		if ( $('#plexapt-our-services').length > 0 ) {
			$('#plexapt-our-services .to-animate').each(function( k ) {
	...
	var servicesWayPoint = function() {
		...
		if ( $('#plexapt-our-services').length > 0 ) {
			$('#plexapt-our-services').waypoint( function( direction ) {
	...
	var featuresAnimate = function() {
		...
		if ( $('#plexapt-features').length > 0 ) {
			$('#plexapt-features .to-animate').each(function( k ) {
	...
	var featuresWayPoint = function() {
		...
		if ( $('#plexapt-features').length > 0 ) {
			$('#plexapt-features').waypoint( function( direction ) {
	...
	var testimonialsAnimate = function() {
		...
		if ( $('#plexapt-testimonials').length > 0 ) {
			$('#plexapt-testimonials .to-animate').each(function( k ) {
	...
	var testimonialsWayPoint = function() {
		...
		if ( $('#plexapt-testimonials').length > 0 ) {
			$('#plexapt-testimonials').waypoint( function( direction ) {
	...
	var pricingAnimate = function() {
		...
		if ( $('#plexapt-pricing').length > 0 ) {
			$('#plexapt-pricing .to-animate').each(function( k ) {
	...
	var pricingWayPoint = function() {
		...
		if ( $('#plexapt-pricing').length > 0 ) {
			$('#plexapt-pricing').waypoint( function( direction ) {
	...
	var pressAnimate = function() {
		...
		if ( $('#plexapt-press').length > 0 ) {
			$('#plexapt-press .to-animate').each(function( k ) {
	...
	var pressWayPoint = function() {
		...
		if ( $('#plexapt-press').length > 0 ) {
			$('#plexapt-press').waypoint( function( direction ) {
